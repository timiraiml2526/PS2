...
# ──────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="PostureSense · Monitor",
    page_icon="🧘",
    layout="wide",
...
# ──────────────────────────────────────────────────────────────────────────────
# GLOBAL SHARED STATE (module-level, survives st.rerun, visible to video thread)
# ──────────────────────────────────────────────────────────────────────────────
if "_ps" not in st.__dict__:
    st._ps = {
        "posture":      "Waiting…",
        "baseline":     None,       # calibration score
        "calib_req":    False,      # set True by UI → consumed by video thread
        "bad_thresh":   0.10,
        "warn_thresh":  0.075,
        "lock":         threading.Lock(),
    }
...
.pill{display:inline-block;padding:.35rem 1.1rem;border-radius:999px;font-family:'Space Mono',monospace;font-size:.82rem;font-weight:700;letter-spacing:.05em;margin-bottom:.8rem}
...
.stButton>button{border-radius:8px!important;font-weight:600!important;transition:opacity .2s}
...
# ──────────────────────────────────────────────────────────────────────────────
_defaults = dict(
    session_active=False, session_start=None,
...
# ──────────────────────────────────────────────────────────────────────────────
# MEDIAPIPE DETECTOR (IMAGE mode — no timestamp headaches)
# ──────────────────────────────────────────────────────────────────────────────
@st.cache_resource
def load_detector():
...
class PostureProcessor(VideoProcessorBase):
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if result.pose_landmarks:
            ...
            # Ear–shoulder vertical score for calibration comparison
            current_score = (
                (l_shldr.y + r_shldr.y) / 2 - (l_ear.y + r_ear.y) / 2
            )

            with _S["lock"]:
                # Consume calibration request
                if _S["calib_req"]:
                    _S["baseline"]  = current_score
                    _S["calib_req"] = False
            ...
            if baseline is not None:
                # Use calibration-relative score
                if current_score < baseline * 0.82:
                    posture = "Bad Posture"
                elif current_score < baseline * 0.91:
                    posture = "Slightly Bent"
                else:
                    posture = "Good Posture"
            else:
                # Fallback: shoulder tilt only
                if tilt > bad_t:
                    posture = "Bad Posture"
                elif tilt > warn_t:
                    posture = "Slightly Bent"
            ...
            color = {
                "Good Posture":  (0, 210, 100),
                "Slightly Bent": (0, 210, 220),
                "Bad Posture":   (0, 0, 220),
            }.get(posture, (120, 120, 120))

            # Draw key landmarks
            KEY_LM = [7, 8, 11, 12, 23, 24]
            for idx in KEY_LM:
                pt = lms[idx]
            ...
            cv2.rectangle(img, (10, 10), (300, 58), bg, -1)
...
with st.sidebar:
    ...
    _thresh = {"Low": (0.13, 0.10), "Medium": (0.10, 0.075), "High": (0.07, 0.05)}
    with _S["lock"]:
        _S["bad_thresh"], _S["warn_thresh"] = _thresh[sensitivity]
...
# ──────────────────────────────────────────────────────────────────────────────
# VOICE ALERT (JS Speech Synthesis via MutationObserver)
# ──────────────────────────────────────────────────────────────────────────────
st.markdown("""
<div id="_ps_voice_trigger" data-key="0"></div>
<script>
(function(){
  var last = "0";
  function fire(el){
  ...
  var el = document.getElementById("_ps_voice_trigger");
  if(el){
    new MutationObserver(function(){ fire(el); }).observe(el,{attributes:true});
  } else {
    document.addEventListener("DOMContentLoaded", function(){
      var e2 = document.getElementById("_ps_voice_trigger");
...
with col_cam:
    ...
    with bc2:
        if st.button("⏹ Stop Session", use_container_width=True):
            ...
            st.rerun()
    with bc3:
        if st.button("🎯 Calibrate", use_container_width=True, help="Sit in good posture then click"):
            with _S["lock"]:
                _S["calib_req"] = True
    ...
    with _S["lock"]:
        b = _S["baseline"]
    if b is not None:
        st.caption(f"🎯 Calibrated — baseline score: `{b:.4f}`")
    else:
        st.caption("⚠️ Not calibrated — click **Calibrate** in good posture for best accuracy.")
...
with col_stats:
    ...
    # Timeline bar chart
    st.markdown("<div class='sh'>Posture Timeline (last 3 min)</div>", unsafe_allow_html=True)
    ...
    if not st.session_state.session_active and st.session_state.session_start and total > 0:
        ...
        grade = ("🏆 Excellent" if good_pct >= 80 else
                 ...
                 "⚠️ Needs Work" if good_pct >= 40 else
                 "❌ Poor")
        for ln in [
        ...
        if good_pct >= 80:
            ...
            st.error("⚠️ Your posture needs attention — see the Analytics page for tips.")
